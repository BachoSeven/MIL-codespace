...
theorem var_derivable {m n : Nat} (v : Nat → Bool) (h1 : m < n) :
  ...
  | succ p ih =>
    ...
    have h1 : m < p + 1 := h1
    -- Ora separiamo i casi m < p e m ≥ p, il secondo implica m = p.
    sorry
...
theorem main_lemma  {φ : Formula} {n : Nat}
  ...
  | neg α ih =>
    ...
    have ih : derivable (row_to_formula n v α) := sorry
    -- Hint: si danno due casi, o la formula α vale true o false
    -- Nota: qui non stiamo usando il terzo escluso. Perche'?
    sorry
  -- Se φ = α → β
  | implies α β ih1 ih2 =>
    show derivable (row_to_formula n v (α → β))
    have ih1 : derivable (row_to_formula n v α) := sorry
    have ih2 : derivable (row_to_formula n v β) := sorry
    -- qui i casi sono quattro...
    sorry
